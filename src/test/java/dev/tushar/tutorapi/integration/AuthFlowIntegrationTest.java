...
class AuthFlowIntegrationTest {
    ...
    @Test
    void register_then_apply_then_admin_approves_then_create_course_then_other_user_enrolls() throws Exception {
        // 1) Tutor-to-be registers as a normal user
        register("ada", "ada@example.com", "Ada", "Lovelace");
        String adaToken = login("ada", "Secret123!");

        // /me has no tutorProfile yet, no admin/tutor authorities
        mockMvc.perform(get("/api/v1/me").header("Authorization", "Bearer " + adaToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.user.username").value("ada"))
                .andExpect(jsonPath("$.data.tutorProfile").doesNotExist());

        // 2) Ada applies to become a tutor
        String applyBody = """
                {"bio":"Math + computing","expertise":"MATH","qualifications":"MSc Math","yearsOfExperience":8,"hourlyRateCents":7500}
                """;
        ...
        // 3) She can't create a course yet — application is PENDING
        mockMvc.perform(post("/api/v1/courses")
                        .header("Authorization", "Bearer " + adaToken)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"title":"Algos","subject":"COMPUTER_SCIENCE","level":"BEGINNER","priceCents":4999}
                                """))
                .andExpect(status().isForbidden());

        // 4) Promote an admin and review the application
        register("root", "root@example.com", "Root", "Admin");
        User admin = userRepository.findByUsername("root").orElseThrow();
        admin.setAdmin(true);
        userRepository.save(admin);
        String adminToken = login("root", "Secret123!");

        // Admin can list applications — used here just to verify access works
        mockMvc.perform(get("/api/v1/admin/tutor-applications?status=PENDING")
                        .header("Authorization", "Bearer " + adminToken))
                .andExpect(status().isOk());

        // Look up Ada's specific application id via her own /me endpoint —
        // avoids data pollution from other tests in the same class.
        ...
        mockMvc.perform(post("/api/v1/admin/tutor-applications/" + applicationId + "/review")
                        .header("Authorization", "Bearer " + adminToken)
                        .contentType(MediaType.APPLICATION_JSON)
        ...
        // 5) Ada re-logs in to pick up the new ROLE_TUTOR authority, then creates a course
        String adaTokenV2 = login("ada", "Secret123!");
        MvcResult courseResult = mockMvc.perform(post("/api/v1/courses")
                        .header("Authorization", "Bearer " + adaTokenV2)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("""
                                {"title":"Algos","subject":"COMPUTER_SCIENCE","level":"BEGINNER","priceCents":4999}
                                """))
        ...
        // 6) A different person registers, enrolls (no role gate), sees their enrollment
        register("grace", "grace@example.com", "Grace", "Hopper");
        String graceToken = login("grace", "Secret123!");
        ...
        mockMvc.perform(post("/api/v1/enrollments")
                ...
                .andExpect(jsonPath("$.data.userName").value("Grace Hopper"));

        mockMvc.perform(get("/api/v1/enrollments/mine")
                        .header("Authorization", "Bearer " + graceToken))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.content[0].courseTitle").value("Algos"));
