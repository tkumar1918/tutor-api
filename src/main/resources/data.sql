...
SELECT 'root', 'root@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Root', 'Admin', NULL, TRUE, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'root');

INSERT INTO users (username, email, password_hash, first_name, last_name, date_of_birth, admin, token_version, created_at, updated_at)
SELECT 'ada', 'ada@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Ada', 'Lovelace', '1990-12-10', FALSE, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'ada');

INSERT INTO users (username, email, password_hash, first_name, last_name, date_of_birth, admin, token_version, created_at, updated_at)
SELECT 'linus', 'linus@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Linus', 'Torvalds', '1985-06-15', FALSE, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'linus');

INSERT INTO users (username, email, password_hash, first_name, last_name, date_of_birth, admin, token_version, created_at, updated_at)
SELECT 'carol', 'carol@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Carol', 'Stein', '1995-03-22', FALSE, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'carol');

INSERT INTO users (username, email, password_hash, first_name, last_name, date_of_birth, admin, token_version, created_at, updated_at)
SELECT 'dave', 'dave@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Dave', 'Cooper', '1998-11-02', FALSE, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'dave');

INSERT INTO users (username, email, password_hash, first_name, last_name, date_of_birth, admin, token_version, created_at, updated_at)
SELECT 'grace', 'grace@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Grace', 'Hopper', '2000-12-09', FALSE, 0, NOW(), NOW()
WHERE NOT EXISTS (SELECT 1 FROM users WHERE username = 'grace');

INSERT INTO users (username, email, password_hash, first_name, last_name, date_of_birth, admin, token_version, created_at, updated_at)
SELECT 'alice', 'alice@example.com', '$2b$10$QTxcc5br7icRwRIe.4IRSe4cFiX7VenTEA0ttUJZTY6yZok58M/v2', 'Alice', 'Wonder', '2001-07-04', FALSE, 0, NOW(), NOW()
...
SELECT u.id, 'Mathematician and computing pioneer. 10 years tutoring experience.', 'MATH', 'MSc Mathematics, University of London; ICM speaker 2018', 10, 7500, 'APPROVED', NOW(), NOW(), (SELECT id FROM users WHERE username = 'root'), NULL, NOW(), NOW()
FROM users u
...
INSERT INTO tutor_profiles (user_id, bio, expertise, qualifications, years_of_experience, hourly_rate_cents, status, applied_at, reviewed_at, reviewed_by_user_id, rejection_reason, created_at, updated_at)
SELECT u.id, 'Software engineer with 25 years of experience. Specializes in systems programming.', 'PROGRAMMING', 'MSc Computer Science, University of Helsinki; kernel maintainer', 25, 10000, 'APPROVED', NOW(), NOW(), (SELECT id FROM users WHERE username = 'root'), NULL, NOW(), NOW()
FROM users u
...
INSERT INTO tutor_profiles (user_id, bio, expertise, qualifications, years_of_experience, hourly_rate_cents, status, applied_at, reviewed_at, reviewed_by_user_id, rejection_reason, created_at, updated_at)
SELECT u.id, 'Chemistry PhD candidate, eager to teach!', 'SCIENCE', 'PhD candidate in Chemistry, MIT', 2, 5000, 'PENDING', NOW(), NULL, NULL, NULL, NOW(), NOW()
FROM users u
...
-- Dave — REJECTED
INSERT INTO tutor_profiles (user_id, bio, expertise, qualifications, years_of_experience, hourly_rate_cents, status, applied_at, reviewed_at, reviewed_by_user_id, rejection_reason, created_at, updated_at)
SELECT u.id, 'I know some French.', 'LANGUAGES', 'High school French B2', 0, 3000, 'REJECTED', NOW(), NOW(), (SELECT id FROM users WHERE username = 'root'), 'Insufficient teaching experience. Please reapply with credentials.', NOW(), NOW()
FROM users u
...
SELECT tp.id, 'Intro to Algorithms', 'Sorting, searching, complexity. A gentle introduction to algorithmic thinking.', 'COMPUTER_SCIENCE', 'BEGINNER', 4999, NOW(), NOW()
...
INSERT INTO courses (tutor_profile_id, title, description, subject, level, price_cents, created_at, updated_at)
SELECT tp.id, 'Advanced Linux Internals', 'Process scheduling, memory management, kernel hacking.', 'COMPUTER_SCIENCE', 'ADVANCED', 9999, NOW(), NOW()
FROM tutor_profiles tp
JOIN users u ON u.id = tp.user_id
WHERE u.username = 'linus'
...
-- Ada's courses
INSERT INTO courses (tutor_profile_id, title, description, subject, level, price_cents, created_at, updated_at)
SELECT tp.id, 'Calculus 101', 'Limits, derivatives, integrals — the calculus foundations.', 'MATH', 'BEGINNER', 2999, NOW(), NOW()
FROM tutor_profiles tp
JOIN users u ON u.id = tp.user_id
WHERE u.username = 'ada'
...
INSERT INTO courses (tutor_profile_id, title, description, subject, level, price_cents, created_at, updated_at)
SELECT tp.id, 'Linear Algebra', 'Vectors, matrices, eigenvalues. The math behind ML.', 'MATH', 'INTERMEDIATE', 3999, NOW(), NOW()
FROM tutor_profiles tp
JOIN users u ON u.id = tp.user_id
WHERE u.username = 'ada'
...
SELECT u.id, c.id, NOW(), 'ACTIVE', NOW(), NOW()
FROM users u
...
INSERT INTO enrollments (user_id, course_id, enrolled_at, status, created_at, updated_at)
SELECT u.id, c.id, NOW(), 'COMPLETED', NOW(), NOW()
FROM users u
CROSS JOIN courses c
...
INSERT INTO enrollments (user_id, course_id, enrolled_at, status, created_at, updated_at)
SELECT u.id, c.id, NOW(), 'CANCELLED', NOW(), NOW()
FROM users u
CROSS JOIN courses c
...
SELECT u.id, tp.id, 'MATH', 'Hi! I''m struggling with multivariable calculus, weekends preferred.', 'PENDING', NULL, NULL, NOW(), NOW()
FROM users u
...
INSERT INTO tutoring_requests (student_user_id, tutor_profile_id, subject, message, status, tutor_reply, responded_at, created_at, updated_at)
SELECT u.id, tp.id, 'COMPUTER_SCIENCE', 'Need help with C pointers and memory management.', 'ACCEPTED', 'Happy to help — send me your availability and we''ll start next week.', NOW(), NOW(), NOW()
FROM users u
CROSS JOIN tutor_profiles tp
JOIN users tu ON tu.id = tp.user_id
...
INSERT INTO tutoring_requests (student_user_id, tutor_profile_id, subject, message, status, tutor_reply, responded_at, created_at, updated_at)
SELECT u.id, tp.id, 'MATH', 'Looking for daily tutoring at $5/hr.', 'REJECTED', 'Sorry, that''s well below my rate. Best of luck with your search!', NOW(), NOW(), NOW()
FROM users u
CROSS JOIN tutor_profiles tp
JOIN users tu ON tu.id = tp.user_id
